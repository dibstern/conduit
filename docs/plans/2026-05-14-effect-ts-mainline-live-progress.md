...
1. The legacy `startDaemonProcess` bridge still exists in `daemon-main.ts`, but active tests and CLI paths no longer import it. The remaining grep hits are the bridge implementation/comments and the static guard.
