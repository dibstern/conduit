...
| Throwing helpers called from Effect programs | Open | Broad grep triage started. `handleInstanceAdd` now converts legacy instance-manager throws to a typed IPC-boundary Effect failure and `{ ok:false }` response instead of defecting. Startup instance rehydration now classifies expected legacy instance-manager throws as typed rehydration failures and continues. PID write failures now map to `DaemonLifecycleLayerError`. Remaining expected project/instance helper throws need slice-by-slice conversion or reclassification. |
