...
|---|---|---|
| `startDaemonProcess` imported by CLI | Done | Recheck before final guardrail close. CLI foreground and internal `--daemon` child startup now enter through the Effect daemon starter facade instead of importing `daemon-main.js`. |
...
12. Single-owner daemon cutover. Started locally: CLI foreground, internal `--daemon` child startup, instance lifecycle parity, daemon runtime-config status parity, daemon lifecycle integration parity, the shared daemon E2E harness, daemon project-order E2E setup, daemon smart-default unit/E2E setup, direct daemon-server integration cases, and daemon health-check integration now use Effect-backed daemon starter facades instead of importing/calling `startDaemonProcess`; foreground daemon instance IPC now calls Effect-owned instance services through the managed runtime; foreground daemon config status is mirrored from the Effect config ref; smart-default probe/convert is owned by the daemon instance-state Layer; instance health checks are owned by an overridable Effect service; `DaemonLiveOptions.ipcContext` is gone, tagged/legacy IPC socket dispatch now runs through the daemon layer's Effect-owned RPC handlers, and the legacy IPC context adapter is deleted.
