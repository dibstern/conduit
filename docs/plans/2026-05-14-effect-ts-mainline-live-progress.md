...
|---|---|---|
| `startDaemonProcess` imported by CLI | Done | Recheck before final guardrail close. CLI foreground and internal `--daemon` child startup now enter through the Effect daemon starter facade, and the legacy bridge export has been deleted from `daemon-main.ts`. |
...
1. No live guardrail blocker remains. Continue with final docs and verification.
...
13. Final guardrail cleanup. Done locally: static runtime bridge guard now covers `Effect`/`Runtime`/`ManagedRuntime` and object `.runPromise`/`.runSync` calls, with all surviving hits explicitly reclassified.
