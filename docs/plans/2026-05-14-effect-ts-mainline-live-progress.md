...
1. The legacy `startDaemonProcess` bridge still exists for remaining non-CLI parity harnesses until branch-local smoke parity is proven against the Effect-owned graph. Instance lifecycle, daemon runtime-config status, and daemon lifecycle integration parity now run through `startForegroundDaemon`.
...
12. Single-owner daemon cutover. Started locally: CLI foreground, internal `--daemon` child startup, instance lifecycle parity, daemon runtime-config status parity, and daemon lifecycle integration parity now use Effect-backed daemon starter facades instead of importing/calling `startDaemonProcess`; foreground daemon instance IPC now calls Effect-owned instance services through the managed runtime; foreground daemon config status is mirrored from the Effect config ref; `DaemonLiveOptions.ipcContext` is gone, tagged/legacy IPC socket dispatch now runs through the daemon layer's Effect-owned RPC handlers, and the legacy IPC context adapter is deleted.
