...
6. Router service ownership and HTTP runtime boundary. Done locally for daemon and standalone relay HTTP handler ownership; daemon WebSocket upgrade routing no longer uses `Runtime.runPromise`, and daemon HTTP project reads are projected from Effect-owned daemon state instead of a `daemon-main` callback.
