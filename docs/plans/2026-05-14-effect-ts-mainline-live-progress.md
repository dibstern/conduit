...
1. The legacy `DaemonIPCContext` / `buildIPCHandlers` adapter still exists for non-production parity tests; remove or explicitly reclassify it after proving no daemon/relay production path depends on it.
