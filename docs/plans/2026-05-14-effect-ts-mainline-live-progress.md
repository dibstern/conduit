...
1. The legacy `startDaemonProcess` bridge still exists for remaining non-CLI parity harnesses until branch-local smoke parity is proven against the Effect-owned graph. Instance lifecycle parity now runs through `startForegroundDaemon`.
...
12. Single-owner daemon cutover. Started locally: CLI foreground, internal `--daemon` child startup, and instance lifecycle parity now use Effect-backed daemon starter facades instead of importing/calling `startDaemonProcess`; foreground daemon instance IPC now calls Effect-owned instance services through the managed runtime; `DaemonLiveOptions.ipcContext` is gone, tagged/legacy IPC socket dispatch now runs through the daemon layer's Effect-owned RPC handlers, and the legacy IPC context adapter is deleted.
