...
1. The legacy `startDaemonProcess` bridge still exists for remaining non-CLI parity harnesses until branch-local smoke parity is proven against the Effect-owned graph. Instance lifecycle and daemon runtime-config status parity now run through `startForegroundDaemon`.
