...
2. The production daemon bridge still exists until the CLI foreground facade and branch-local smoke parity are proven against the Effect-owned graph.
...
12. Single-owner daemon cutover. Started locally: CLI foreground now uses an Effect-backed foreground starter/facade and no longer calls `startDaemonProcess`; the `--daemon` child path remains the production CLI import blocker until its IPC context is Effect-owned.
