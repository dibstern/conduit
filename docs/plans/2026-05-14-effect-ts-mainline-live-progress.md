...
| Throwing helpers called from Effect programs | Open | Broad grep triage started. `handleInstanceAdd` now converts legacy instance-manager throws to a typed IPC-boundary Effect failure and `{ ok:false }` response instead of defecting. Startup instance rehydration now classifies expected legacy instance-manager throws as typed rehydration failures and continues. PID write failures now map to `DaemonLifecycleLayerError`. Expected project-registry lifecycle failures and daemon ready-relay/not-found helper failures now use tagged registry domain errors. Remaining expected instance helper throws need slice-by-slice conversion or reclassification. |
