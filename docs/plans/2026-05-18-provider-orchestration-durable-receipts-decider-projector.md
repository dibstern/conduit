...
- [ ] Modify: `src/lib/provider/event-sink.ts` and `src/lib/provider/relay-event-sink.ts` if Phase 5 chooses `ProviderRuntimeIngestion` as the single provider-output ingress.
...
- [ ] Phase 0, preflight/spike: fake provider + SQLite + restart/crash windows. Pick receipt states and duplicate return shape before integration. Include an event-log independence spike proving append does not need a pre-existing `sessions` row.
...
- [ ] Phase 2, receipt repository: add command fingerprint, aggregate/session fields, upsert/get, migration tests.
- [ ] Phase 3, decider/projector: model current commands as events/read model without provider calls.
- [ ] Phase 4, engine queue: dispatch checks receipt, decides, appends events, projects read model, writes receipt transactionally.
- [ ] Phase 5, side-effect reactor: provider calls execute from durable requested events; idempotent by command id/sequence; provider output enters through `ProviderRuntimeIngestion` unless a narrower compatibility exception is documented and guarded.
- [ ] Phase 6, handler compatibility: old `dispatchEffect` facade preserves callers until handlers can accept command acknowledgements.
- [ ] Phase 7, cleanup: remove in-memory dedupe/session maps and toy idempotency service.
...
| Provider output has one durable ingress owner. | Reactor/provider sink tests plus static guard. | New production provider output routes through `ProviderRuntimeIngestion`; mapper imports outside ingestion/translator tests are deleted or explicitly reclassified. |
...
- [ ] Provider output bypasses durable ingestion. Prove new production provider side-effect output does not import/call `translateProviderRuntimeEventToDomain` directly and does not append/project by hand.
...
1. Run prereq tests.
2. Complete `docs/plans/2026-05-20-delete-legacy-opencode-runtime-ingress.md`.
3. Spike receipt states, event-log independence, and crash windows with fake provider + SQLite.
4. Add scenario-first durable receipt and event-store independence tests.
5. Remove event-store dependency on read-model/projection rows; remove `ProviderRuntimeIngestion` session pre-seeding.
6. Add receipt repository/migration tests, then implement.
7. Add decider/projector tests for current command behavior.
8. Add queued engine tests for receipt lookup/transaction/read-model update.
9. Add side-effect reactor tests for idempotent provider calls and provider-output ingress convergence.
10. Wire compatibility `dispatchEffect` facade and update handlers incrementally.
11. Remove in-memory dedupe/session maps and toy idempotency service.
12. Run guardrails and verification commands; inspect failures directly.
