...
Use Beads (`bd`) for durable task tracking with the `beads` skill at `~/.agents/skills/beads/SKILL.md` (global install) for Beads workflow guidance, then use the `bd` CLI for issue operations.
