...
import {
	...
	spawnDaemon,
} from "../../../src/lib/daemon/daemon-spawn.js";
...
describe("spawnDaemon port pre-flight check", () => {
	...
	it("throws EADDRINUSE when port is already in use", async () => {
		...
		const rejected = spawnDaemon(
			{ port, host: "127.0.0.1", configDir: "/tmp/conduit-test-spawn" },
			stubIsRunning,
		);

		await expect(rejected).rejects.toBeInstanceOf(DaemonSpawnPortInUseError);
		await expect(rejected).rejects.toMatchObject({
			_tag: "DaemonSpawnPortInUseError",
			host: "127.0.0.1",
			port,
		});
		await expect(rejected).rejects.toThrow("EADDRINUSE");
	});
