...
// has been appended and projected, and the create and fork calls, which are
// id-generating and so cannot be expressed as commands on their own.
		...
		const outsideTheSeam = productionSourceFiles(SRC_ROOT)
			...
			.flatMap(({ file, path }) =>
				readFileSync(file, "utf8")
					.split("\n")
					.flatMap((line, index) =>
						/api\.session\.(?:create|update|delete|fork)\s*\(/.test(line)
...
Every session create, fork, rename, or delete goes through applySessionCommand in ${SESSION_COMMAND_SEAM}, which appends the canonical event, projects it, and only then tells the provider. Route the mutation through the seam rather than adding an exception here.
...
See docs/adr/0004-session-mutations-are-canonical-events.md.`;
	...
	it("keeps the seam itself honest: the mutations are still there to confine", () => {
		...
		const mutations = seam.match(
			/api\.session\.(?:create|update|delete|fork)\s*\(/g,
		);
