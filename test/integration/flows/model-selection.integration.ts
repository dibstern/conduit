...
describe("Integration: Model Selection", () => {
	...
	it("switch_model updates model_info broadcast", async () => {
		...
		const modelInfo = await client.waitFor("model_info", { timeout: 10_000 });
	...
	it("message succeeds after model switch", async () => {
		...
		await client.waitFor("model_info", { timeout: 10_000 });
		client.clearReceived();
		...
		// Wait for delta first (proves SSE pipeline works), then done.
		// The done event is generated by the status poller which consumes
		// GET /session/status queue entries independently — allow enough
		// time for it to reach the busy→idle transition.
	...
	it("new_session resets model selection without breaking messages", async () => {
		...
		const evidence = await Promise.race([
			client.waitFor("done", { timeout: 10_000 }),
			client.waitFor("result", { timeout: 10_000 }),
		]);
